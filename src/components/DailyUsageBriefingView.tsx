...
export function DailyUsageBriefingView({ hidden, onNavigate }: { hidden: boolean; onNavigate: (view: string) => void }) {
  ...
  const [events, setEvents] = useState<UsageAnalyticsEvent[]>([]);
  ...
  const loadEvents = async () => { setEventsLoading(true); try { setEvents(await loadUsageAnalyticsEvents()); } catch { setNotice("Detailed analytics history is not available in this runtime yet."); } finally { setEventsLoading(false); } };
  ...
  return <div className="tray-content" hidden={hidden}><section className="repo-intelligence-view" aria-labelledby="daily-briefing-title">
    ...
    {!loading && briefing && briefing.completeness !== "insufficient-data" ? <>
      ...
      <article className="repo-map-panel"><div className="repo-map-panel__header"><h2>Detailed local events</h2><button className="secondary-button secondary-button--small" onClick={() => void loadEvents()} disabled={eventsLoading} type="button">{eventsLoading ? "Loading…" : "Load events"}</button></div>{events.length ? <ul className="repo-map-tool-list">{events.slice(0, 12).map((event) => <li className="repo-map-tool-list__item repo-map-tool-list__item--ok" key={event.id}><span>{event.label} · {event.source}</span><small>{new Date(event.occurredAt).toLocaleString()} · {event.requestCount} request{event.requestCount === 1 ? "" : "s"} · {event.inputTokens} input · {event.savedTokens} saved · {event.confidence}{event.outcome ? ` · ${event.outcome}` : ""}</small></li>)}</ul> : <p>Load bounded, normalized event facts. Prompts, responses, paths, headers, and credentials are never stored.</p>}</article>
