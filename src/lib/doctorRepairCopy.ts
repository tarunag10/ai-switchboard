...
export function doctorRepairHint(action: string): string {
  ...
  switch (action) {
    ...
    case "install_repo_memory_mcp":
      return "Installs the app-managed read-only Repo Memory MCP server. Then click Start MCP in Mode Inspector to verify repo_context_pack, repo_symbol_lookup, and repo_dependents_of.";
    default:
...
export function doctorIssueGuidance(issue: DoctorIssue): string {
  ...
  switch (issue.id) {
    ...
    case "repo_memory_mcp_not_configured":
      return "Install Repo Memory MCP from Doctor, then click Start MCP in Mode Inspector before asking supported agents to consume repo-memory tools.";
