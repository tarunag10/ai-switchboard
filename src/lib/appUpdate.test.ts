...
describe("app update helpers", () => {
  ...
  it("returns the install progress copy for the selected update", () => {
    expect(getAppUpdateInstallStatusCopy(availableUpdate)).toBe("Downloading AI Switchboard for Mac 0.3.0…");
  ...
  it("marks updates as ready to restart after a successful install", async () => {
    ...
    expect(result).toEqual({
      readyToRestart: true,
      showDialog: true,
      statusCopy: "AI Switchboard for Mac 0.3.0 is installed and ready to restart.",
    });
  });
  ...
  it("formats download progress with byte counts and percent when total is known", () => {
    ...
    ).toBe("Downloading AI Switchboard for Mac 0.3.0: 5.5 MB of 22.0 MB (25%)…");
  });
