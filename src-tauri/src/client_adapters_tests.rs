...
fn planned_connectors_are_detected_but_not_enabled_or_verified() {
    ...
    for connector in planned {
        ...
        let dry_run_copy = format!("{} {}", dry_run.detail, dry_run.required_evidence.join(" "));
        for snippet in [
            "target path",
            "before/after",
    ...
    for connector in connectors.iter() {
        ...
        assert!(
            !preview.target.trim().is_empty(),
            "{} preview target",
            connector.client_id
        );
        assert!(
            !preview.marker.trim().is_empty(),
            "{} preview marker",
            connector.client_id
        );
        assert!(
            !preview.backup_path.trim().is_empty(),
            "{} preview backup",
            connector.client_id
        );
        assert!(
            !preview.current_state.trim().is_empty(),
            "{} preview current state",
            connector.client_id
        );
        assert!(
            !preview.proposed_state.trim().is_empty(),
            "{} preview proposed state",
            connector.client_id
        );
        assert!(
            !preview.apply_blocked_reason.trim().is_empty(),
            "{} preview block reason",
            connector.client_id
        );
        assert!(
            !preview.rollback_preview.trim().is_empty(),
            "{} preview rollback",
            connector.client_id
        );
        assert!(
            !preview.confirmation_phrase.trim().is_empty(),
...
fn parse_json_object_accepts_json5_but_rejects_non_objects() {
    ...
    let err = parse_json_object("[]", Path::new("settings.json")).expect_err("arrays are rejected");
    assert!(err
        .to_string()
...
fn managed_block_upsert_replaces_existing_block_without_duplication() {
    ...
    assert_eq!(
        content
            .matches("# >>> ai-switchboard:claude_code >>>")
            .count(),
        1
    );
...
"#;
    ...
    assert_eq!(
        updated
            .matches("# >>> ai-switchboard:managed_rtk >>>")
            .count(),
        1
    );
    assert_eq!(
...
fn strip_hook_removes_headroom_entry_and_leaves_other_entries() {
    ...
    fs::write(&settings, serde_json::to_string_pretty(&content).unwrap()).expect("write settings");
...
fn gemini_setup_writes_verifies_and_cleans_sidecar_only() {
    ...
    assert_eq!(
        fs::read_to_string(&sidecar).expect("read gemini after second off"),
        cleaned_once
    );
}
...
fn gemini_verification_fails_closed_for_each_drifted_shell_export() {
    ...
    let shell_paths = applied
        ...
        .filter(|path| {
            path.file_name()
                .is_some_and(|name| name != SWITCHBOARD_ROUTING_FILE)
        })
        .collect::<Vec<_>>();
    assert!(
        !shell_paths.is_empty(),
        "apply should report Gemini shell targets"
    );
    let exports = [
        (
...
fn aider_sidecar_lifecycle_applies_repairs_rolls_back_and_disables() {
    ...
    assert!(
        crate::aider_provider_configs::aider_provider_config_matches()
    ...
    let preview = super::preview_managed_rollback("aider-routing").expect("preview aider rollback");
    assert_eq!(preview.status, ManagedRollbackExecutionStatus::Ready);
    assert!(preview.marker_present);
    assert_eq!(
        preview.confirmation_phrase,
    ...
    let provider_rollback = super::preview_managed_rollback("aider-provider-routing")
        .expect("preview aider provider rollback");
    assert_eq!(
        provider_rollback.status,
        ManagedRollbackExecutionStatus::Ready
    );
    assert_eq!(
    ...
    let verification = super::verify_client_setup("aider").expect("verify disabled aider setup");
    assert!(!verification.verified);
...
fn continue_sidecar_lifecycle_applies_repairs_rolls_back_and_disables() {
    ...
    assert!(config_content.contains(super::HEADROOM_OPENAI_BASE_URL));
    assert!(
        crate::continue_provider_configs::continue_provider_config_matches()
    ...
    let provider_rollback = super::preview_managed_rollback("continue-provider-routing")
...
fn goose_native_provider_and_mcp_bridge_lifecycle_preserves_user_state() {
    ...
    let preview = super::preview_managed_rollback("goose-routing").expect("preview goose rollback");
    assert_eq!(preview.status, ManagedRollbackExecutionStatus::Ready);
    assert!(preview.marker_present);
    assert_eq!(
        preview.confirmation_phrase,
    ...
    let verification = super::verify_client_setup("goose").expect("verify disabled goose setup");
    assert!(!verification.verified);
}
...
fn goose_and_grok_sidecar_apply_requires_exact_current_confirmation_and_preserves_user_content() {
    ...
    for (record_id, client_id, owner) in [
        ...
        assert!(
            super::execute_managed_config_apply(record_id, &preview.confirmation_phrase).is_err()
        );
        assert_eq!(
        ...
        let applied = super::execute_managed_config_apply(record_id, &preview.confirmation_phrase)
...
fn managed_rollback_undo_all_executes_ready_native_rows_only() {
    ...
    let result =
        super::execute_managed_rollback_undo_all("Undo all ready Switchboard native rollback rows")
...
fn apply_then_verify_then_disable_codex_round_trip() {
    ...
    let verification = super::verify_client_setup("codex").expect("verify_client_setup succeeds");
...
fn apply_codex_is_byte_idempotent() {
    ...
    assert_eq!(
        toml_second
            .matches("# >>> ai-switchboard:codex_cli >>>")
            .count(),
        1,
...
fn managed_rollback_preview_and_execute_restores_opencode_backup() {
    ...
    let preview = super::preview_managed_rollback("opencode-routing").expect("preview rollback");
...
fn managed_config_apply_preview_and_execute_promotes_opencode_safely() {
    ...
    let preview = super::preview_managed_config_apply("opencode-routing").expect("preview apply");
    assert_eq!(preview.status, ManagedRollbackExecutionStatus::Ready);
...
fn managed_config_apply_preview_and_execute_promotes_zed_rollback_safely() {
    ...
    let preview = super::preview_managed_config_apply("zed-ai-routing").expect("preview zed apply");
...
fn managed_config_apply_rejects_opencode_drift_after_preview() {
    ...
    let err = super::execute_managed_config_apply("opencode-routing", &preview.confirmation_phrase)
...
fn grok_native_endpoint_setup_preserves_config_and_supports_rollback_and_off_cleanup() {
    ...
    let preview = super::preview_managed_config_apply("grok-routing").expect("native Grok preview");
    assert_eq!(preview.status, ManagedRollbackExecutionStatus::Ready);
...
fn qwen_connector_applies_and_disables_switchboard_owned_sidecar_only() {
    ...
    let rollback =
        super::preview_managed_rollback("qwen-code-routing").expect("preview qwen rollback");
    assert_eq!(rollback.status, ManagedRollbackExecutionStatus::Ready);
    ...
    assert!(
        super::apply_client_setup("qwen_code")
            .expect("repair qwen sidecar")
            .verification
            .verified
    );
    super::disable_client_setup("qwen_code").expect("disable repaired qwen sidecar");
    let repaired_cleaned =
        std::fs::read_to_string(&routing_path).expect("read repaired qwen sidecar");
    super::disable_client_setup("qwen_code").expect("repeat disable qwen sidecar");
    assert_eq!(
        std::fs::read_to_string(&routing_path).expect("read repeated qwen off"),
        repaired_cleaned
    );
}
