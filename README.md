...
| Aider, Continue | Managed native + sidecar | Yes | No | Yes | Allowlisted provider routing plus Switchboard-owned routing markers with Doctor verification, rollback, and Off cleanup; credentials/account state remains manual. |
| Qwen Code, Amazon Q Developer CLI, Zed AI | Managed sidecar/editor | Yes | No | Yes | Switchboard-owned routing markers with Doctor verification, rollback, and Off cleanup; provider/account state remains manual. |
...
npm run switchboard -- repo-intelligence <repo-path> --start-session --agent codex --budget 6000 --format markdown
```
...
npm run repo:intelligence -- <repo-path> --start-session --agent codex --task verification --budget 6000 --format markdown
