...
- Doctor now blocks repair actions that can restore Headroom routing while the saved mode is Off or RTK-only.
- Doctor exposes Verify Off as a primary one-click action when active routing evidence remains.
- Mode Inspector now tells users to restart stale terminals/editors when old `ANTHROPIC_BASE_URL`, `OPENAI_BASE_URL`, or `PATH` exports may survive a mode change.
...
- Add an app restart smoke test that saves Off mode, quits, relaunches, and verifies no proxy. Local ad-hoc evidence is generated by `npm run smoke:mode-relaunch:local -- --confirm`; reboot and signed-release evidence remain separate.
- Document that already-running shells may retain old environment variables until those shells restart: shipped in Mode Inspector attention guidance.
