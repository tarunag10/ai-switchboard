...
- **Dependency call-graph analyzers:** language-specific helpers such as TypeScript compiler APIs, Rust metadata, Python AST/import scanners, shell workflow parsers, and deeper package manifest readers. Current `path-graph-v5` support links shell script invocations to local scripts, Python absolute and relative local imports to indexed modules, and TypeScript, JavaScript, and React external imports back to `package.json`.
...
- Reuse the managed connector readiness contract for Gemini CLI, OpenCode, Cursor, Grok / xAI CLI, Aider, Continue, Goose, Qwen Code, Amazon Q Developer CLI, Windsurf, and Zed AI: identify config surfaces, document manual-safe workflows, keep sidecar lifecycle coverage visible, and block native provider/editor config mutation behind backup/restore/off-mode gates.
