...
Repo Intelligence is underway, not complete. The app now has a read-only foundation for local file classification, rough token estimation, and bounded implementation, verification, and handoff context packs. It does not yet have a complete Graphy-style symbol graph, call graph, dependency graph, persistent index, or in-app context-pack workflow.
...
1. **Read-only local indexer**
   - First foundation: run `npm run repo:intelligence -- <repo-path>` to produce local file roles, token estimates, and bounded context packs.
   - Next: scan symbols, imports, routes, tests, package manifests, high-churn files, and ownership hints.
...
3. **UI workflow**
   - Add Repo Intelligence add-on card install, index, re-index, pause, and remove-index controls.
