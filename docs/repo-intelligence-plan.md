...
- **Dependency call-graph analyzers:** language-specific helpers such as TypeScript compiler APIs, Rust metadata, Python AST/import scanners, and deeper package manifest readers. Current `path-graph-v4` support links Python absolute and relative local imports to indexed modules, and links TypeScript, JavaScript, and React external imports back to `package.json`.
