...
4. **Agent integrations**
  - Expose context packs through local CLI/MCP-style commands that Claude Code, Codex, Gemini CLI, OpenCode, and similar tools can call. Current CLI contract includes `npm run repo:intelligence -- <repo> --manifest` for pack discovery and read-only `apiQueries`, `--list-api` for local API query command names, `--list-agents` for supported handoff ids, read-only `agentRecipes` for Claude Code, Codex, Gemini CLI, OpenCode, Aider, Goose, Qwen Code, Amazon Q Developer CLI, Cursor, Continue, Windsurf, and Zed AI, `--pack <id> --format markdown` for task-specific handoff, `--agent <claude|codex|gemini|opencode|aider|goose|cursor|continue|grok|qwen|amazonq|windsurf|zed> --format markdown` for ready-to-paste bounded tool handoffs, `--agent <id> --format json` for machine-readable `mac_ai_switchboard.repo_agent_handoff` payloads, and `--session --agent <id> --task <implementation|verification|handoff|risk_review|release_handoff>` for `mac_ai_switchboard.agent_session_preparation` output with freshness, copy status, recommended mode, and selected handoff.
  - `get_agent_handoff` and matching CLI handoff exports include planned connector config readiness when the target is Gemini CLI, OpenCode, Cursor, Grok / xAI CLI, Aider, Continue, Goose, Qwen Code, Amazon Q Developer CLI, Windsurf, or Zed AI. The readiness block carries the next gate, evidence requirements, config path strategy, account caveat, and rollback strategy while keeping automation disabled.
