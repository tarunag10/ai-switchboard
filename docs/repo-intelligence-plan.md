...
- **Dependency call-graph analyzers:** language-specific helpers such as TypeScript compiler APIs, Rust metadata, Python AST/import scanners, and deeper package manifest readers. Current `path-graph-v3` package-manifest support links TypeScript, JavaScript, and React external imports back to `package.json`.
