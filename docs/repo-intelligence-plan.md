...
Repo Intelligence is underway, not complete. The app now has a read-only foundation for local file classification, rough token estimation, bounded implementation, verification, and handoff context packs. It now includes dependency hubs, path-based import/dependency edges, content-derived import references, tree-sitter-assisted import and call references for supported source languages, package-dependency edges from external imports to `package.json`, reverse dependency hubs, a bounded symbol graph, symbol-level caller-to-callee edges for parsed TypeScript/JavaScript/React, Rust, and Python call expressions, a read-only local API manifest in the local graph summary, and a repo-memory MCP stdio transport with app-managed install/start/stop smoke coverage and service issue diagnostics. The call graph remains intentionally bounded and name-resolved (it does not claim whole-program type or dynamic dispatch resolution), and the plan still does not complete a persistent parser index, long-running MCP daemon supervision, or writable agent automation.
...
- **Graphy-style repo graph:** Symbol/call graph view for files, call paths, dependency hubs, and richer test relationships. Current implementation already includes bounded path-based dependency edges, content-derived import references, tree-sitter-assisted multiline imports, symbol-level caller-to-callee call edges, Python local import-reference edges, reverse dependency hubs, and symbol extraction for TypeScript, JavaScript, Rust, Python, Swift, Markdown, CSS, and HTML source.
...
- **Dependency call-graph analyzers:** language-specific helpers such as TypeScript compiler APIs, Rust metadata, Python AST/import scanners, shell workflow parsers, Markdown heading scanners, stylesheet/HTML asset parsers, and deeper package manifest readers. Current `path-graph-v10` support links visible test/source relationships, tree-sitter-assisted TypeScript/JavaScript/React multiline imports and AST call references, bounded static imported-alias and namespace-member resolution for TypeScript/JavaScript/React, Python, and Rust, bounded three-hop named/wildcard local re-export resolution with a strict deeper-chain cutoff, Rust/Python parser-assisted import scans, Markdown headings into symbols with parent hierarchy, shell script invocations to local scripts, `package.json` scripts to local shell scripts and script-to-script chains, Python absolute and relative local imports to indexed modules, stylesheet `@import`/`url(...)` references, HTML script/style/image links, and TypeScript, JavaScript, and React external imports back to `package.json`.
...
4. **Agent integrations**
  ...
  - `get_agent_handoff` and matching CLI handoff exports include managed connector config readiness when the target is Gemini CLI, OpenCode, Cursor, Grok / xAI CLI, Aider, Continue, Goose, Qwen Code, Amazon Q Developer CLI, Windsurf, or Zed AI. The readiness block carries the next gate, evidence requirements, config path strategy, account caveat, and rollback strategy; Goose and Grok/xAI native adapters are limited to their documented allowlisted endpoint fields while unsupported provider/editor mutation stays gated.
