...
- MarkItDown cleanup now reports orphaned Read-hook removal truthfully and fails with context if the hook cannot be removed; focused native coverage proves removal is idempotent and change-aware.
- Chonkify is now usable from the optimization UI for its approved narrow scope: explicit local activation controls Repo Intelligence pack compression only, synchronizes with the Repo Intelligence selector, preserves source spans and native fallback, and labels savings estimated; provider traffic remains untouched.
- MarkItDown disable/uninstall now removes its app-owned temporary conversion cache as well as the hook, permission, and managed blocks; cleanup is bounded, symlink-aware, idempotent, and disclosed in the privacy notice.
