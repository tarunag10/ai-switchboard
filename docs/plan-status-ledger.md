...
- Public release proof now reconciles completed live release assets separately from remaining proof: `npm run release:proof` records the `v0.0.0` signed/notarized DMG and checksum as completed live evidence while machine-checking updater `latest.json`, workflow/default updater endpoint reachability, updater `.sig` release assets, current checkout static/installed smoke summaries, and reboot-level installed proof as separate blockers.
