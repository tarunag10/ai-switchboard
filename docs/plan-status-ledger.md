...
- Cursor, Goose, and Grok/xAI Switchboard-owned sidecar lifecycles are complete with dry-run preview, exact confirmation, sibling backup, disk verification, rollback, and Off cleanup. Native provider/account/model settings remain gated pending stable allowlisted schemas.
...
- Native/provider write promotion for Cursor, Goose, and Grok/xAI remains gated until verified allowlisted schemas and full lifecycle fixtures exist. Their Switchboard-owned sidecars are complete; provider/account/model state remains manual.
