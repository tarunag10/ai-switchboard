...
Treat the build as blocked until `release:report` is clear, the DMG is signed/notarized, `/Applications/Mac AI Switchboard.app` exists from the DMG install, the beta smoke checklist has been run on that installed app, and `npm run smoke:installed -- --confirm` has written `dist/installed-smoke-summary.md`.
