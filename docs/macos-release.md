...
export HEADROOM_UPDATER_ENDPOINTS='["https://github.com/tarunag10/ai-switchboard/releases/latest/download/latest.json"]'
...
Treat the build as blocked until `npm run release:ready -- --strict` is clear, the DMG is signed/notarized, the canonical bundle `/Applications/AI Switchboard.app/Contents/Info.plist` exists from the DMG install, the beta smoke checklist has been run on that installed app, and `MAC_AI_SWITCHBOARD_PUBLIC_ARTIFACT_PATH=/absolute/path/to/the-tested.dmg npm run smoke:installed -- --confirm` has written `dist/installed-smoke-summary.md`. The installed smoke summary records the SHA-256 of both `docs/beta-smoke-test.md` and the tested DMG; rerun installed smoke whenever either changes so release readiness does not accept stale or unrelated evidence.
...
Website and release-page download CTAs should present the product as **AI Switchboard for Mac** and link to the GitHub Release asset. Keep the compatibility DMG filename `Mac-AI-Switchboard_<version>.dmg` and the existing updater endpoint stable; the installed bundle path is the canonical `/Applications/AI Switchboard.app`.
...
- production builds fall back to `https://github.com/tarunag10/ai-switchboard/releases/latest/download/latest.json` when no explicit updater env vars are injected
