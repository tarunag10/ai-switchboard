...
2. **Repo Intelligence depth**: symbol-level caller-to-callee AST call edges now cover TypeScript/JavaScript/React, Rust, and Python with mixed-language fixtures, including bounded static imported-alias and namespace-member resolution. Repo Map now exposes typed per-tool progress/current-tool evidence plus explicit cancellation, overlap protection, opt-in CLI retries, and UI retry controls. Whole-program type inference and dynamic dispatch remain intentionally outside the local bounded resolver.
