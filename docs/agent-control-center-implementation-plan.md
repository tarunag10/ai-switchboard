...
- Rollback Center UI now exposes the allowlisted Codex, Gemini, OpenCode, and sidecar-backed native flows: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore/cleanup button, and result/error copy.
...
- Turn repo-memory MCP from a smoke-tested CLI transport into a fully app-managed local service. The status/copy contract, backend install/start/stop commands, Mode Inspector actions, Doctor repair integration, and connector-consumption matrix exist; remaining work is real long-running service supervision and connector-specific MCP bridge setup docs as native/MCP-aware agents are promoted.
...
- Finish exact live/session savings attribution across MarkItDown, Ponytail, Caveman, and Compact Chinese by adding durable per-session source events where profile-specific evidence exists. Headroom and RTK measured session events are shipped; Repo Intelligence now records estimated best-pack token-avoidance events after successful index builds; the shared durable attribution event contract accepts MarkItDown, Ponytail, Caveman, and Compact Chinese sources with measured, estimated, or inferred confidence.
...
- Durable savings attribution now records estimated Repo Intelligence best-pack token-avoidance events after successful index builds, and the schema accepts source-specific events for MarkItDown, Ponytail, Caveman, and Compact Chinese plus measured/estimated/inferred confidence values.
