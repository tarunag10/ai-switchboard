...
- Rollback Center now has a guarded execution-preview contract for every managed record, plus native undo-all preview/execution for backend-allowlisted Codex, Gemini, and OpenCode rows that separates executable rows from manual or cleanup-only rows, including stable undo-all order, exact confirmation phrase, backend action classification, cleanup-only boundaries, disk re-read relaunch-survival evidence for native rows, and native-write status while unsupported restore commands remain blocked.
- Backend Rollback Center execution now has allowlisted `codex-routing` and `opencode-routing` restore paths plus Gemini managed-block cleanup: preview validates marker/backup readiness where required, execute requires the exact confirmation phrase, backup restores create a fresh safety backup, cleanup removes only Switchboard-owned Gemini routing blocks, and fixture-home tests cover the native rows.
- Rollback Center UI now exposes native restore/cleanup controls only for the backend-allowlisted Codex, Gemini, and OpenCode rows: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore/cleanup button, native undo-all preview/execution for ready rows, and result/error copy. Cursor, Grok/xAI CLI, Aider, Continue, Goose, Qwen Code, Amazon Q, Windsurf, and Zed remain visible as rollback plans/manual rows until backend execution exists.
