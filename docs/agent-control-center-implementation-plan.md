...
- Rollback Center now has a guarded execution-preview contract for every managed record, plus a copyable undo-all preview that separates executable Codex/OpenCode rows from manual or cleanup-only rows, including stable undo-all order, exact confirmation phrase, backend action classification, cleanup-only boundaries, and native-write status while unsupported restore commands remain blocked.
- Backend Rollback Center execution now has allowlisted `codex-routing` and `opencode-routing` restore paths: preview validates marker/backup readiness, execute requires the exact confirmation phrase and sibling backup, creates a fresh safety backup, and fixture-home tests cover restore, wrong-backup rejection, and missing-marker rejection for both rows.
- Rollback Center UI now exposes the allowlisted Codex and OpenCode native restore flows: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore button, and restore/error result copy.
...
- Complete persistent Rollback Center execution with actual per-change restore actions, relaunch survival, and a guarded "undo all Switchboard changes" flow. The Doctor copy surface, inventory model, execution-preview contract, copyable undo-all preview, and Codex/OpenCode backend/frontend restore paths are shipped; remaining work is executable undo-all orchestration, relaunch-survival evidence, and promotion to other config-backed rows.
