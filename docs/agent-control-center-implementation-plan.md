...
Current checkpoint: Start Agent Session, connector readiness coverage, promoted Gemini/OpenCode/Windsurf/Zed managed routing, Goose's allowlisted native endpoint plus managed read-only Repo Memory MCP bridge, Grok/xAI's allowlisted native endpoint routing, read-only Repo Intelligence APIs, app-owned repo-memory MCP lifecycle/supervision evidence, durable savings attribution, guarded Rollback Center native restore/cleanup, native undo-all for ready rows, the Vite/Vercel browser boundary, release-readiness local evidence, Caveman Compact Chinese, and trust-hardening guards are shipped. Remaining work centers on provider-specific native config mutation for Cursor and other still-gated connectors, signed/notarized public release evidence, installed-app relaunch survival, deeper Repo Intelligence graphs, stronger measured add-on counters, real approved config writes, and final trust hardening.
...
- Connector readiness coverage exists for Gemini CLI, OpenCode, Grok/xAI CLI, Cursor, Aider, Continue, Goose, Qwen Code, Amazon Q, Windsurf, and Zed, including config surfaces, manual guides, automation gates, rollback dossiers, copyable config-plan actions, compatibility-matrix checks, dry-run previews for detected or documented config surfaces, and a seven-stage automation path for detect, dry-run, backup, apply, verify, rollback, and Off cleanup. Gemini CLI, OpenCode, Windsurf, and Zed AI have promoted managed routing; Goose and Grok/xAI have promoted allowlisted native endpoint routing (Goose also exposes managed Repo Memory MCP); Cursor, Aider, Continue, Qwen Code, and Amazon Q retain sidecar/readiness coverage while unsupported native/provider writes stay gated. The managed manifest set is Claude Code, Codex, Gemini CLI, OpenCode, Goose, Grok/xAI, Windsurf, and Zed AI.
...
- Safe config dry-run coverage proves managed write paths can produce blocked previews with target, backup, marker, rollback, Off cleanup, and unmanaged-config boundaries; confirmed apply plans now require the exact preview phrase before producing write-ready next text. OpenCode, Windsurf, and Zed now have promoted native safe-apply paths with backend preview, exact confirmation, sibling backup, write, verify, and Rollback Center restore coverage.
...
- Rollback Center now has a guarded execution-preview contract for every managed record, plus native undo-all preview/execution for backend-allowlisted Codex, Gemini, OpenCode, Windsurf, Zed, and managed connector sidecar rows that separates executable rows from dedicated cleanup-only rows, including stable undo-all order, exact confirmation phrase, backend action classification, cleanup-only boundaries, disk re-read relaunch-survival evidence for native rows, and native-write status while unsupported restore commands remain blocked.
- Backend Rollback Center execution now has allowlisted `codex-routing`, `opencode-routing`, `windsurf-routing`, and `zed-ai-routing` restore paths, Gemini managed-block cleanup, and managed sidecar cleanup for Cursor, Grok/xAI CLI, Aider, Continue, Goose, Qwen Code, and Amazon Q: preview validates marker/backup readiness where required, execute requires the exact confirmation phrase, backup restores create a fresh safety backup, cleanup removes only Switchboard-owned blocks, and fixture-home tests cover the native rows.
- Rollback Center UI now exposes native rollback controls for backend-allowlisted Codex, Gemini, OpenCode, Windsurf, Zed, managed sidecar rows, dedicated managed-storage cleanup, Repo Intelligence summary cleanup, LaunchAgent/login-item cleanup, app-state cleanup, and Ponytail receipt/plugin cleanup: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore/cleanup button, native undo-all preview/execution for ready config rows, dedicated cleanup confirmation flows, and result/error copy.
...
- Promote provider-specific config mutation beyond the current allowlisted Goose/Grok endpoint fields, connector by connector, only after each native surface has real backup, apply, verify, rollback, Doctor repair, fixture-home restore tests, and Off cleanup. Priority surfaces include Cursor editor settings, Continue multi-provider config, Aider wrapper/env config, Qwen account/model guardrails, and Amazon Q credential-safe integration; Windsurf and Zed already have promoted managed editor-settings routing and remain reference paths for editor-style native writes.
- Extend the app-managed repo-memory MCP service with signed installed-app relaunch survival evidence. The status/copy contract, backend install/start/stop commands, Mode Inspector actions, Doctor repair integration, connector-consumption matrix, connector-specific bridge setup recipes, persisted supervision status, child/restart/exit evidence, last-check evidence, and stale-config detection are shipped; only installed-app/reboot proof remains external.
- Deepen Repo Intelligence v2 with tree-sitter or language-specific parsers, richer symbols/imports/reverse dependencies, and graph-aware packs. Persistent parser/index version fields, health fields, Doctor health checks, expanded lightweight symbol extraction for modern JS/TS/Python async declarations, and bounded symbol-level caller-to-callee AST edges for TypeScript/JavaScript/React, Rust, and Python are shipped. Whole-program type/dynamic-dispatch resolution and long-running analyzers remain roadmap work.
...
- Promote confirmed safe config apply plans into real user-approved file writes, verification, and rollback flows for supported connectors without touching unmanaged config. OpenCode provider config plus Windsurf/Zed editor settings are promoted; remaining connectors still need the same safe path before native writes are enabled.
- Complete persistent Rollback Center execution with actual per-change restore actions, relaunch survival, and a guarded "undo all Switchboard changes" flow. The Doctor copy surface, inventory model, execution-preview contract, copyable undo-all preview, Codex/OpenCode/Windsurf/Zed backup restore paths, Gemini managed cleanup path, sidecar-backed connector cleanup paths, managed-storage cleanup, Repo Intelligence summary cleanup, LaunchAgent/login-item cleanup, app-state cleanup, Ponytail receipt/plugin cleanup, UI execution gate for no-backup cleanup rows, guarded native undo-all orchestration for ready rows, and disk re-read relaunch-survival evidence are shipped; remaining work is installed-app relaunch smoke evidence and promotion to deeper provider-specific config-backed rows.
...
- Gemini CLI, OpenCode, Grok/xAI CLI, Cursor, Aider, Continue, Goose, Qwen Code, Amazon Q, Windsurf, and Zed now have connector readiness lifecycle coverage with config surfaces, manual guides, automation gates, rollback dossiers, dry-run previews, backup/apply/verify/rollback/Off cleanup stages, and release-readiness evidence. Gemini/OpenCode/Windsurf/Zed expose promoted managed routing evidence; Goose and Grok/xAI expose promoted allowlisted native endpoint evidence, while Goose separately exposes managed read-only Repo Memory MCP; Cursor and the remaining connectors keep unsupported native/provider writes gated.
- `npm run check:connectors` reports the manifest-managed count separately from managed connector dossiers, promoted sidecar dossiers, gated native-write dossiers, and retained compatibility dossiers, and enforces the shared config-creation contract, including the seven required gated steps and copyable markdown handoff.
...
- Managed connector cards now provide per-tool Copy config plan actions, so Gemini, OpenCode, Grok/xAI, Cursor, Aider, Continue, Goose, Qwen Code, Amazon Q, Windsurf, and Zed each export the same native config-creation contract from the Mac app. Goose's provider-routing contract is limited to its documented allowlisted endpoint fields; remaining provider/account/model mutation stays gated while its Repo Memory MCP bridge is managed.
...
- Repo Memory MCP docs now include connector-specific bridge setup recipes for Claude Code, Goose, Cursor/Windsurf/Zed, Continue/Aider, and Gemini/OpenCode/Grok/Qwen/Amazon Q, with deployment readiness checks requiring the read-only and no-provider-mutation boundaries.
...
- Expose read-only local repo context through app UI, CLI, connector bridge recipes, and the repo-memory MCP interface so agents stop rediscovering the same codebase; remaining MCP work is long-running supervision.
...
Current baseline: Claude Code, Codex, Gemini CLI, OpenCode, Goose, Grok/xAI CLI, Windsurf, and Zed AI are manifest-managed. Gemini CLI uses managed shell routing, OpenCode uses promoted provider-config routing, Windsurf/Zed use promoted editor-settings routing, Goose uses allowlisted native endpoint routing plus a managed read-only Repo Memory MCP bridge, and Grok/xAI uses allowlisted `models_base_url` routing. Cursor, Aider, Continue, Qwen Code, and Amazon Q Developer CLI retain sidecar/readiness lifecycle coverage until their native/provider writes are proven. `npm run check:connectors` should continue to report manifest-managed connectors separately from managed connector dossiers, promoted sidecar dossiers, gated native-write dossiers, and retained compatibility dossiers.
