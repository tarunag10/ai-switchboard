...
- Repo-memory MCP now has an app-facing lifecycle status contract in the Mode Inspector, Doctor repair flow, and Doctor timeline copy, including direct install actions, smoke-check command, read-only tool names, and secret-exclusion safety boundary.
...
- Backend Rollback Center execution has started with an allowlisted `codex-routing` restore path: preview validates marker/backup readiness, execute requires the exact confirmation phrase and sibling backup, creates a fresh safety backup, and fixture-home tests cover restore, wrong-backup rejection, and missing-marker rejection.
- Rollback Center UI now exposes the allowlisted Codex native restore flow: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore button, and restore/error result copy.
...
- Turn repo-memory MCP from a smoke-tested CLI transport into a fully app-managed local service. The status/copy contract, backend install command, Mode Inspector install action, Doctor repair integration, and first connector-consumption docs exist; remaining work is direct start/stop controls.
...
- Complete persistent Rollback Center execution with actual per-change restore actions, relaunch survival, and a guarded "undo all Switchboard changes" flow. The Doctor copy surface, inventory model, execution-preview contract, and first Codex backend/frontend restore path are shipped; remaining work is undo-all orchestration, relaunch-survival evidence, and promotion to other config-backed rows.
