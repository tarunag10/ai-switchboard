...
- Complete persistent Rollback Center execution with actual per-change restore actions, relaunch survival, and a guarded "undo all Switchboard changes" flow. The Doctor copy surface, inventory model, and execution-preview contract are shipped; next backend slice is `preview_managed_rollback` plus `execute_managed_rollback` for one config-backed row with fixture-home restore/rejection tests.
