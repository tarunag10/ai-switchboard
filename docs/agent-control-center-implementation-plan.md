...
- Repo-memory MCP now has an app-facing lifecycle status contract in the Mode Inspector and Doctor timeline copy, including the app install action, smoke-check command, read-only tool names, and secret-exclusion safety boundary.
...
- Turn repo-memory MCP from a smoke-tested CLI transport into a fully app-managed local service. The status/copy contract and backend install command exist; remaining work is direct UI install/start/stop controls, Doctor repair integration, and agent-consumption docs for supported connector paths.
