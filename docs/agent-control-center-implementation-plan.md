...
- Repo-memory MCP now has an app-facing lifecycle status contract in the Mode Inspector, Doctor repair flow, and Doctor timeline copy, including direct install/start/stop actions, smoke-check command, read-only tool names, persisted app-session active state, and secret-exclusion safety boundary.
...
- Durable savings attribution has started with append-only measured Headroom engine session events exposed through a read-only backend command; the frontend session ledger now consumes those measured events when available. RTK, Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese remain gated until source-specific event capture exists.
...
- Rollback Center now has a guarded execution-preview contract for every managed record, plus a copyable undo-all preview that separates executable Codex/Gemini/OpenCode rows from manual or cleanup-only rows, including stable undo-all order, exact confirmation phrase, backend action classification, cleanup-only boundaries, and native-write status while unsupported restore commands remain blocked.
- Backend Rollback Center execution now has allowlisted `codex-routing` and `opencode-routing` restore paths plus `gemini-routing` managed-block cleanup: preview validates marker/backup readiness where required, execute requires the exact confirmation phrase, backup restores create a fresh safety backup, Gemini cleanup removes only Switchboard-owned shell/sidecar blocks, and fixture-home tests cover the native rows.
- Rollback Center UI now exposes the allowlisted Codex, Gemini, and OpenCode native flows: preview readiness, target/backup/marker evidence, exact confirmation input, guarded restore/cleanup button, and result/error copy.
- Release readiness surfaces have shipped report loading, one-click fresh report refresh through `npm run release:ready -- --json`, executable desktop validation, static smoke-preflight, and local installed-smoke evidence generation, copyable snapshots, command copy, source labels that distinguish guidance from report proof, evidence coverage summary, next blocked release action, connector-readiness evidence checks, and separate signing/notarization/updater blocker categories.
...
- Turn repo-memory MCP from a smoke-tested CLI transport into a fully app-managed local service. The status/copy contract, backend install/start/stop commands, Mode Inspector actions, Doctor repair integration, and first connector-consumption docs exist; remaining work is real long-running service supervision and connector-specific consumption docs as MCP-aware agents are promoted.
- Deepen Repo Intelligence v2 with tree-sitter or language-specific parsers, richer symbols/imports/reverse dependencies, graph-aware packs, and parser/index Doctor health checks. Persistent parser/index version and health fields are shipped.
- Finish exact live/session savings attribution across RTK, Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese by adding durable per-session source events where profile-specific evidence exists.
...
- Complete persistent Rollback Center execution with actual per-change restore actions, relaunch survival, and a guarded "undo all Switchboard changes" flow. The Doctor copy surface, inventory model, execution-preview contract, copyable undo-all preview, Codex/OpenCode backup restore paths, and Gemini managed cleanup path are shipped; remaining work is executable undo-all orchestration, relaunch-survival evidence, and promotion to other config-backed rows.
- Complete release readiness execution by wiring signed/local-DMG build/install and confirmed public installed-smoke evidence generation into the in-app panel. Fresh report refresh from `npm run release:ready -- --json`, desktop validation, static smoke-preflight execution, and local installed-smoke evidence recording are shipped.
