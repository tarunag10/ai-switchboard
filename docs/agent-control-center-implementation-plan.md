...
- Durable savings attribution has started with append-only measured Headroom engine and RTK session events exposed through a read-only backend command; the frontend session ledger now consumes those measured events when available. Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese remain gated until source-specific event capture exists.
...
- Finish exact live/session savings attribution across Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese by adding durable per-session source events where profile-specific evidence exists. Headroom and RTK measured session events are shipped.
