...
- Finish exact live/session savings attribution across Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese by adding durable per-session source events where profile-specific evidence exists. Headroom and RTK measured session events are shipped; the shared durable attribution event contract now accepts Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese sources with measured, estimated, or inferred confidence.
...
- Durable savings attribution schema now accepts source-specific events for Repo Intelligence, MarkItDown, Ponytail, Caveman, and Compact Chinese, plus measured/estimated/inferred confidence values, with backend decode coverage and frontend ledger replacement coverage.
