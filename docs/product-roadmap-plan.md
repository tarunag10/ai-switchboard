...
- The remaining connector risk is provider-specific native config mutation for tools whose routing is not yet managed: Cursor, Continue, Aider, Qwen Code, and Amazon Q still need connector-specific safe writes. Goose and Grok / xAI CLI now have allowlisted native endpoint adapters with fixture lifecycle proof; credentials, account state, and model selection remain manual. Gemini CLI, OpenCode, Windsurf, and Zed AI remain reference paths for promoted managed routing, while Aider/Qwen/Amazon Q retain managed sidecar cleanup.
