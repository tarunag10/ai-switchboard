...
- Repo Intelligence graph version `path-graph-v5` adds shell script invocation edges on top of Python local import-reference edges and TypeScript/JavaScript/React package-dependency edges back to `package.json`, so context packs can show shell workflow relationships, Python module relationships, and source-to-installed-package relationships in addition to local imports and call references.
