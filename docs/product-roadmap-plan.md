...
- Repo Intelligence graph version `path-graph-v10` adds bounded static imported-alias and namespace-member resolution on top of tree-sitter-assisted multiline imports and AST call-reference edges.
...
- Connector readiness coverage exists for Gemini CLI, OpenCode, Grok/xAI CLI, Cursor, Aider, Continue, Goose, Qwen Code, Amazon Q, Windsurf, and Zed. Gemini CLI, OpenCode, Aider, Continue, Windsurf, and Zed AI have promoted managed routing evidence; Goose and Grok/xAI CLI have promoted allowlisted native endpoint evidence (Goose also has a managed read-only Repo Memory MCP bridge); Cursor, Qwen Code, and Amazon Q retain sidecar/readiness coverage until additional native/provider writes are proven. The managed manifest set is Claude Code, Codex, Gemini CLI, OpenCode, Goose, Grok/xAI, Windsurf, and Zed AI.
...
- Repo Intelligence graph version `path-graph-v10` adds bounded static imported-alias and namespace-member resolution for TypeScript/JavaScript/React, Python, and Rust on top of tree-sitter-assisted multiline import and AST call references, stylesheet and HTML asset dependency edges and CSS/HTML symbols, visible test/source relationships, Markdown heading symbols with parent hierarchy, shell script invocation edges, Python local import-reference edges, and TypeScript/JavaScript/React package-dependency edges back to `package.json`, so context packs can show web entrypoint links, verification links, docs anchors, shell workflow relationships, Python module relationships, parser-derived call sites, and source-to-installed-package relationships in addition to local imports and bounded fallback text references.
...
- Gemini CLI, OpenCode, Aider, Continue, Windsurf, Zed AI, Goose, and Grok / xAI CLI now have promoted managed routing surfaces with backup, verification, rollback, and Off cleanup evidence. Aider is limited to `openai-api-base`, Continue to one managed `config.yaml` model entry, Goose to documented OpenAI/Anthropic endpoint fields alongside its read-only Repo Memory MCP bridge, and Grok to `endpoints.models_base_url`. Qwen Code and Amazon Q Developer CLI retain managed Switchboard-owned sidecar lifecycles while provider/account state remains manual.
- The remaining connector risk is provider-specific native config mutation for tools whose routing is not yet managed: Cursor, Qwen Code, and Amazon Q still need connector-specific safe writes. Aider and Continue now have allowlisted native/provider adapters with fixture lifecycle proof; credentials, account state, and unrelated model settings remain manual. Goose and Grok / xAI CLI retain allowlisted native endpoint adapters, while Gemini CLI, OpenCode, Windsurf, and Zed AI remain promoted managed-routing reference paths.
