...
- Rollback Center has guarded preview/execution for backend-allowlisted rows, including Codex/OpenCode restore paths, Gemini managed-block cleanup, managed sidecar cleanup rows, and dedicated Repo Intelligence summary cleanup. Dedicated app-state, storage, launch-agent, and plugin cleanup rows stay visible as rollback plans/manual rows until backend execution exists.
...
- Broader Rollback Center native restore coverage for dedicated cleanup domains beyond the shipped Codex/OpenCode/Gemini/sidecar and Repo Intelligence summary rows.
