...
- Repo Intelligence graph version `path-graph-v7` adds visible test/source relationships and Markdown heading symbols with parent hierarchy on top of shell script invocation edges, Python local import-reference edges, and TypeScript/JavaScript/React package-dependency edges back to `package.json`, so context packs can show verification links, docs anchors, shell workflow relationships, Python module relationships, and source-to-installed-package relationships in addition to local imports and call references.
...
- Home and Usage savings surfaces now include per-client session trends for Claude Code, Codex, and future connected tools, showing requests, spent tokens, saved tokens, estimated USD, and reduction from current app-session usage events.
