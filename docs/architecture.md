...
- Managed connectors are visible and repairable. Claude Code, Codex, Gemini CLI, OpenCode, Aider, Continue, Windsurf, Zed AI, Goose, and Grok / xAI CLI have managed setup/verify/repair coverage for their proven routing surfaces; Aider is limited to `openai-api-base`, Continue to one managed `config.yaml` model entry, Goose's native writes to documented OpenAI/Anthropic endpoint fields plus the read-only Repo Memory MCP bridge, and Grok's to `endpoints.models_base_url`. Cursor, Qwen Code, and Amazon Q Developer CLI remain guided or detected until their native surfaces satisfy the same backup, restore, Doctor verification, rollback, and Off-mode cleanup contract. Each connector carries a readiness contract: config surfaces the app may inspect, safe Switchboard modes available today, automation gates, the current manual workflow, and the first safe automation step.
...
Gemini CLI, OpenCode, Windsurf, Zed AI, Goose's allowlisted endpoint fields,
and Grok/xAI's documented `models_base_url` are the promoted native-routing
surfaces beyond Claude Code and Codex. Gemini has managed shell base-url env
routing, OpenCode has provider config routing, Windsurf/Zed have editor
settings routing, and Goose also carries a read-only Repo Memory MCP bridge;
all are behind the shared lifecycle. Remaining gated connectors stay manual
...
- Add a proper repo picker and richer index status UI around the existing cache,
  parser-health, skipped-file, and graph-input evidence.
- Deepen semantic graph resolution beyond the bounded, name-resolved symbol
  edges (for example, whole-program types and dynamic dispatch) without
  weakening the read-only boundary.
- Extend connector bridge recipes only where a documented, safe provider schema
  exists. The current app-owned MCP lifecycle is supervised during its app
  session; OS-level daemon/reboot survival remains a release-proof gate.
- Add explicit cleanup/off-mode behavior for any future Repo Intelligence hooks.
- Keep Rust formatting and desktop tests in release/CI gates; local shells without
  `cargo` must rely on CI for backend validation.
