...
- CLI: `npm run repo:intelligence -- <repo-path>` prints file roles, rough token estimates, and implementation, verification, and handoff packs. `--manifest` emits an agent-readable JSON contract with pack ids, commands, graph availability, estimated token savings, and read-only safety flags.
  Agents can request one task-specific Markdown pack with `npm run repo:intelligence -- <repo-path> --pack implementation --format markdown`, or a ready-to-paste tool handoff with `npm run repo:intelligence -- <repo-path> --agent gemini`. Supported handoff targets are `gemini`, `opencode`, `aider`, `goose`, `cursor`, `continue`, and `grok`.
