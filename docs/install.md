...
5. `docs/beta-smoke-test.md` has been run against the installed app, then `npm run smoke:installed` records `dist/installed-smoke-summary.md`, including Switchboard modes, Doctor, planned connector evidence, Repo Intelligence recipes, per-tool agent handoffs, and Codex compression recovery.
