...
| Aider | Managed sidecar connector | External CLI | Strong for Switchboard-owned sidecar routing and repo packs | User-installed CLI detected read-only; Switchboard writes managed sidecar markers only | Managed | Include | Sidecar lifecycle is backed by Doctor verification, rollback, and Off cleanup; saved provider config remains manual. |
| Continue | Managed sidecar connector | External editor extension | Strong for Switchboard-owned sidecar routing; multi-provider config parsing remains manual | User-installed config detected read-only; Switchboard writes managed sidecar markers only | Managed | Include | Sidecar lifecycle is backed by Doctor verification, rollback, and Off cleanup; extension/provider lists remain manual. |
| Goose | Managed MCP connector | External CLI | Strong for repo packs and MCP handoff | User-installed CLI detected read-only; Switchboard writes managed Repo Memory MCP bridge metadata only | Managed MCP | Include | Provider routing remains manual; managed evidence covers the Repo Memory MCP bridge boundary, Doctor verification, rollback, and Off cleanup. |
| Qwen Code | Managed sidecar connector | External CLI | Strong for Switchboard-owned sidecar routing and repo packs; provider/account compatibility stays visible | User-installed CLI detected read-only; Switchboard writes managed sidecar markers only | Managed | Include | Sidecar lifecycle is backed by Doctor verification, rollback, and Off cleanup; provider routing stays manual. |
| Amazon Q Developer CLI | Managed sidecar connector | External CLI | Strong for Switchboard-owned sidecar routing and verification packs; AWS credential state stays outside Switchboard | User-installed CLI detected read-only; Switchboard writes managed sidecar markers only | Managed | Include | Sidecar lifecycle is backed by Doctor verification, rollback, and Off cleanup without reading AWS secrets or SSO cache. |
